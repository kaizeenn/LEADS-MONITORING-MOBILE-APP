...
export default function Dashboard() {
  ...
  // Set default division for karyawan on load
  useEffect(() => {
    if (user && user.role === 'karyawan') {
      setActiveDivisi(user.bagian || 'marketing');
    } else {
    ...
    <div className={`dashboard-wrapper theme-${activeDivisi}`}>
      ...
      <header className="header">
        ...
        <div className="user-toolbar">
          ...
          <button onClick={logout} className="btn btn-outline" style={{ padding: '8px 16px', fontSize: '13.5px' }}>
            <LogOut size={16} />
            <span>Keluar</span>
          </button>
        </div>
